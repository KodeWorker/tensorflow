...
BundleWriterDIT::BundleWriterDIT(Env* env, StringPiece prefix, const Options& options)
    : env_(env),
      options_(options),
      prefix_(prefix),
...
Status BundleWriterDIT::AddSlice(StringPiece full_tensor_key,
                              const TensorShape& full_tensor_shape,
...
Status BundleWriterDIT::Finish() {
  ...
  status_ = errors::Internal("BundleWriterDIT is closed");
  return Status::OK();
}
...
bool BundleReaderDIT::Contains(StringPiece key) {
  Seek(key);
...
Status BundleReaderDIT::LookupTensorShape(StringPiece key, TensorShape* shape) {
  DataType ignored;
  return LookupDtypeAndShape(key, &ignored, shape);
}

string BundleReaderDIT::DebugString() {
